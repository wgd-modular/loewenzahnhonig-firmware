...
static ReverbSc DSY_SDRAM_BSS
verb;
#define MAX_DELAY static_cast<size_t>(48000 * 2.5f)
static DelayLine<float, MAX_DELAY> DSY_SDRAM_BSS
dell;
static DelayLine<float, MAX_DELAY> DSY_SDRAM_BSS
delr;
static Svf filtL;
...
void AudioCallback(AudioHandle::InputBuffer in, AudioHandle::OutputBuffer out, size_t size) {
    for (size_t i = 0; i < size; i++) {
        ...
        pitchShifterLR =
                (pitchShifter1 * (0.5 * (pot4Value * pot4Value))) + (pitchShifter2 * (0.2 * (pot4Value * pot4Value)));
